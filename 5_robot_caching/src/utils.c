...
IPTR AllocateIndividuals(int howmany, int lchrom) {
	...
	for (i = 0; i < howmany; i++) {
		...
		if (pj->chrom == NULL) return NULL;

		pj->backup = (ChromTypeB *) malloc(sizeof(ChromTypeB) * lchrom);
		if (pj->backup == NULL) return NULL;
		pj->chromLen = lchrom;
	}
	return tmp;
}
...
void strcreate(char **dest, char *src) {
	...
	*dest = (char *) malloc(sizeof(char) * strlen(src) + 1);
	if (!*dest) { perror("strcreate"); }
	strcpy(*dest, src);
}

void Shuffle(int *deck, int size) {
	int i;
	for (i = 0; i < size; i++) { deck[i] = i; /* Deal a deck ! */ }
...
double IntPow(int pow, int to) {
	...
	prod = 1.0;
	for (i = 0; i < to; i++) { prod *= (double) pow; }
...
{
	int i;
	for (i = 0; i < size; i++) { gene[i] = bits[i]; }
}
...
void IntSort(int *inarray, int howmany) {
	...
	for (current = 0; current < howmany - 1; current++) {
		...
		for (i = current + 1; i < howmany; i++) {
			if (inarray[i] < inarray[max]) { max = i; }
		}
