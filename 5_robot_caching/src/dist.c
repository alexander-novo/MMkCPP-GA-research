...
void SetInserted(ChromType *tmplate, ChromType *_new, ChromType *p1, ChromType *p2, int tlen,
                 int p1len, int p2len, int chromLen);
...
double HamDist(ChromType *p1, ChromType *p2, int size1, int size2, ChromType **tmplate) {
	...
	for (i = 0; i < size1; i++) { dist += (p1[i] == p2[i] ? 0 : 1); }
	return (double) dist;
}
...
/* LCS is in lcs.c */
double LCS(ChromType *p1, ChromType *p2, int size1, int size2, ChromType **tmplate) {
	int length, dist;
...
void FixTemplate(ChromType **tmplate, int tlen, ChromType *p2, int p2len, ChromType *p1,
                 int chromLen)
/* I want tmplate to be the right size for this problem (chromLen)
Note length of template MUST be <= chromLen */
{
	int i;
	ChromType *_new, *p1Copy, *p2Copy;
	ChromType *pt;

	pt     = *tmplate;
	_new   = (ChromType *) calloc((size_t) chromLen, sizeof(ChromType));
	p1Copy = (ChromType *) calloc((size_t) chromLen, sizeof(ChromType));
	p2Copy = (ChromType *) calloc((size_t) p2len, sizeof(ChromType));

	for (i = 0; i < tlen; i++) { _new[i] = pt[i]; }
	for (i = 0; i < chromLen; i++) { p1Copy[i] = p1[i]; }
	for (i = 0; i < p2len; i++) { p2Copy[i] = p2[i]; }

	if (p2len > chromLen) {
		SetInserted(pt, _new, p2Copy, p1Copy, tlen, p2len, chromLen, chromLen);
	} else {
		SetInserted(pt, _new, p1Copy, p2Copy, tlen, chromLen, p2len, chromLen);
	}
	free(*tmplate);
	*tmplate = _new;
...
void SetInserted(ChromType *tmplate, ChromType *_new, ChromType *p1, ChromType *p2, int tlen,
	...
	for (i = 0; i < chromLen; i++) {
		if (Member(_new[i], tmplate, tlen) < 0) {
			j = 0;
			while (((_new[i] = GetRemoveUnused(p1, p1len)) >= chromLen) && (j++ < chromLen))
				;
		}
	}

	//  HelpPrint(stdout, _new, chromLen, "Fix_new");
...
void HelpPrint(FILE *fp, ChromType *ch, int len, char *name) {
	int i;
	fprintf(fp, "%s :", name);
	for (i = 0; i < len; i++) { fprintf(fp, " %i", ch[i]); }
	fprintf(fp, "\n");
}
