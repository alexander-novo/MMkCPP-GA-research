...
void Copy(ChromType *child, ChromType *parent, int lchrom) {
	int i;
	for (i = 0; i < lchrom; i++) { child[i] = parent[i]; }
}
...
void SwapMutate(ChromType *ci1, ChromType *ci2, int lchrom, float pMut) {
	int i;
	for (i = 0; i < lchrom; i++) {
		if (Flip(pMut)) { SwapChromType(&ci1[Rnd(0, lchrom - 1)], &ci1[Rnd(0, lchrom - 1)]); }
		if (Flip(pMut)) { SwapChromType(&ci2[Rnd(0, lchrom - 1)], &ci2[Rnd(0, lchrom - 1)]); }
	}
}
...
void Slide(ChromType *c1, int pick, int target, int lchrom) {
	...
	if (pick < target) {
		for (i = pick; i < target; i++) { c1[i] = c1[i + 1]; }
		c1[target] = tmp;
	} else if (target < pick) {
		for (i = pick; i > target; i--) { c1[i] = c1[i - 1]; }
		c1[target] = tmp;
...
void Invert(ChromType *ci, int start, int end) {
	int i;
	int mid = (start + end) / 2;
	for (i = start; i < mid; i++) { SwapChromType(&ci[i], &ci[end - (i - start)]); }
}
