...
int dijkstraRouteTable[NUMBER_POINTS][NUMBER_POINTS]
                      [MAX_ROUTE_LENGTH];  // with very large graphs this
                                           // exceeds memory limits for created
                                           // executables

// int ***dijkstraRouteTable; //dynamically allocate to get past static memory
// limit (with very large arrays)
...
const double DEADHEADING_RATIO =
    1.0;  // Ratio of deadheading cost to service cost (can be adjusted to
          // reflect real speeds of robot)
const int NUM_PERMUTATIONS = 1;

// int DIJKSTRA_COUNT = 0; //count then number of times Dikstra's algorithm was
// called
int INIT_COUNT = 0;

int totalLength;

typedef struct {  // struct to return cost and route of vertices at once in
                  // Dijkstra
  int dist;
...
int Dijkstra(int start, int end, int route[MAX_ROUTE_LENGTH]) {
  ...
  {
    if (dijkstraDistTable[start][end] <
        -1)  // if this occurs the dist table is not being stored properly
    {
    ...
    for (int i = 0; i < MAX_ROUTE_LENGTH; i++) {
      /*if (dijkstraRouteTable[start][end][i] < -1)  //if this occurs the route
      table is not being stored properly
      {
              printf("Less than 1:\n");
              printf("Dijkstra route table value: %d at i=%d\n",
      dijkstraDistTable[start][end], i); printf("Dijkstra route table value - 1:
      %d at i=%d\n", dijkstraDistTable[start][end] - 1, i); printf("Dijkstra
      route table value + 10: %d at i=%d\n", dijkstraDistTable[start][end] + 10,
      i);
      }
      ...
      // printf("route[i]: %d\n", route[i]);
      // printf("I got there.\n\n");
      // route[i] = -1;  // this doesn't crash when the program is called from
    ...
    for (int j = 0; j < NUMBER_POINTS; j++) {
      if (vis[j] == false && graph[start][j] > 0) {
        dis[j] =
            graph[start][j] *
            visitedGraph[start][j];  // Multiply distance by 1.0 if unvisited,
                                     // by deadheading ratio if visited --
      }
    }
    ...
    for (int i = 0; i < NUMBER_POINTS; i++) {
      ...
      for (int j = 0; j < NUMBER_POINTS; j++) {
        if (vis[j] == false && graph[minmark][j] > 0 &&
            dis[j] >
                dis[minmark] +
                    (graph[minmark][j] *
    ...
    // Reconstruct route (produces route in reverse order: destination to
    // source)
    int u = end;
    int routeIndex = 0;
    if (prev[u] != -1 || u == start) {
...
int getStartPoint(IPTR pj, int p0,
                  int p1) {  // p0=30, p1=31   -- returning too high value
  // printf("getStartPoint\n");
  int p0s = edges[pj->chrom[p0]]
                .start;  // p0s = start point for edge. p0e = end point for edge
  int p0e = edges[pj->chrom[p0]].end;
  int p1s = edges[pj->chrom[p1]].start;
  int p1e = edges[pj->chrom[p1]]
                .end;  // posibly going past the max length since robots index
  ...
  } else if (p0e == p1s || p0e == p1e)  // if 0e=1s or 0e=1e
  {
    ...
    int _max = max(max(max(d00, d01), d10), d11);  // shouldn't this be min?
    if (d00 == _max || d01 == _max) {  // if 0s - 1s or 1e is longest start at
                                       // 0s
      return p0s;
    } else if (d10 == _max ||
               d11 == _max) {  // if 0e - 1s/1e is longest start at 0e
      return p0e;
    }
  }
      ...
      *currentPoint = edges[pj->chrom[i]].end;
      ...
      for (int z = 0; z < MAX_ROUTE_LENGTH - 1; z++) {
        if (dummyRoute[z] != -1 && dummyRoute[z + 1] != -1) {
          ...
          // //mark edge as visited visitedGraph[dummyRoute[z +
          // 1]][dummyRoute[z]] = DEADHEADING_RATIO;
        }
      }
      ...
      *currentPoint = edges[pj->chrom[i]].start;
      ...
      for (int z = 0; z < MAX_ROUTE_LENGTH - 1; z++) {
        if (dummyRoute2[z] != -1 && dummyRoute2[z + 1] != -1) {
          // visitedGraph[dummyRoute2[z]][dummyRoute2[z + 1]] =
          // DEADHEADING_RATIO;  //mark edge as visited
          // visitedGraph[dummyRoute2[z + 1]][dummyRoute2[z]] =
          // DEADHEADING_RATIO;
        }
      }
    }
    currentDistance +=
        graph[edges[pj->chrom[i]].start][edges[pj->chrom[i]].end] *
        visitedGraph[edges[pj->chrom[i]].start]
                    [edges[pj->chrom[i]].end];  // mult with visited graph to
                                                // account for deadheading --
    // visitedGraph[edges[pj->chrom[i]].start][edges[pj->chrom[i]].end] =
    // DEADHEADING_RATIO;   //mark edge as visited
    // visitedGraph[edges[pj->chrom[i]].end][edges[pj->chrom[i]].start] =
...
void getRobot12(IPTR pj, int *robot1, int *robot2, int *robot3, int *robot4,
                int *robot5) {  // modified
...
double Eval(IPTR pj)  // modified 83-87 are robots
{
  ...
  for (int x = 0; x < NUM_PERMUTATIONS; x++) {
    ...
    if (robot2 - robot1 <= 2 || (robot2 - robot1) >= pj->chromLen - 2 ||
        robot3 - robot2 <= 2 || robot3 - robot2 >= pj->chromLen - 2 ||
        robot4 - robot3 <= 2 || robot4 - robot3 >= pj->chromLen - 2 ||
        robot5 - robot4 <= 2 || robot5 - robot4 >= pj->chromLen - 2 ||
        robot5 - robot1 >= pj->chromLen - 2 || robot1 == 1) {
      return 9000000000000;
    }

    int currentPoint1, currentPoint2, currentPoint3, currentPoint4,
        currentPoint5, currentPoint6;
    double robot1Dis, robot2Dis, robot3Dis, robot4Dis, robot5Dis;

    // printf("\n-----\n");
    for (int y = 0; y < 5; y++) {
      if (myOrder[y] == 0) {
        currentPoint1 = getStartPoint(pj, (robot1 + 1) % pj->chromLen,
                                      (robot1 + 2) % pj->chromLen);
        ...
        {
                printf("\nROBOT's route length less than that expressed in
        chromosome. \n"); printf("\n chromDist: %f, routeDist: %f \n",
        chromDist, robot1Dis); printf("\n chromSegment: "); index = (robot1 + 1)
        % (pj->chromLen); while (index != robot2)
                {
                        printf("%d ", pj->chrom[index]);
                        printf("( %d, %d ), ", edges[pj->chrom[index]].start,
        edges[pj->chrom[index]].end); index = (index + 1) % (pj->chromLen);
                }
                printf("\n\n route1: ");
        ...
        currentPoint2 = getStartPoint(pj, (robot2 + 1) % pj->chromLen,
                                      (robot2 + 2) % pj->chromLen);
        ...
        if (robot2Dis < chromDist)
        {
                printf("\nROBOT's route length less than that expressed in
        chromosome. \n"); printf("\n chromDist: %f, routeDist: %f \n",
        chromDist, robot2Dis); printf("\n chromSegment: "); index = (robot2 + 1)
        % (pj->chromLen); while (index != robot3)
                {
                        printf("%d ", pj->chrom[index]);
                        printf("( %d, %d ), ", edges[pj->chrom[index]].start,
        edges[pj->chrom[index]].end); index = (index + 1) % (pj->chromLen);
        ...
        currentPoint3 = getStartPoint(pj, (robot3 + 1) % pj->chromLen,
                                      (robot3 + 2) % pj->chromLen);
        ...
        if (robot3Dis < chromDist)
        {
                printf("\nROBOT's route length less than that expressed in
        chromosome. \n"); printf("\n chromDist: %f, routeDist: %f \n",
        chromDist, robot3Dis); printf("\n chromSegment: "); index = (robot3 + 1)
        % (pj->chromLen); while (index != robot4)
                {
                        printf("%d ", pj->chrom[index]);
                        printf("( %d, %d ), ", edges[pj->chrom[index]].start,
        edges[pj->chrom[index]].end); index = (index + 1) % (pj->chromLen);
        ...
        currentPoint4 = getStartPoint(pj, (robot4 + 1) % pj->chromLen,
                                      (robot4 + 2) % pj->chromLen);
        // route distance from robot1+1 to robot2
        Tuple r4;
        ...
        if (robot4Dis < chromDist)
        {
                printf("\nROBOT's route length less than that expressed in
        chromosome. \n"); printf("\n chromDist: %f, routeDist: %f \n",
        chromDist, robot4Dis); printf("\n chromSegment: "); index = (robot4 + 1)
        % (pj->chromLen); while (index != robot5)
                {
                        printf("%d ", pj->chrom[index]);
                        printf("( %d, %d ), ", edges[pj->chrom[index]].start,
        edges[pj->chrom[index]].end); index = (index + 1) % (pj->chromLen);
        ...
        currentPoint5 = getStartPoint(pj, (robot5 + 1) % pj->chromLen,
                                      (robot5 + 2) % pj->chromLen);
        ...
        // if robot5 is the last element in the chromosome, there won't be a
        // part1
        if (robot5 != pj->chromLen - 1) {
          // r5part1 = routeDistance(pj, (robot5 + 1) % pj->chromLen,
          // pj->chromLen, &currentPoint5);
          r5part1 = PhenoRouteGet(pj, (robot5 + 1) % pj->chromLen, pj->chromLen,
                                  &currentPoint5);
        ...
        // if robot1 is the first element in the chromosome, there won't be a
        // part2
        if (robot1 != 0) {
        ...
        if (robot5Dis < chromDist)
        {
                printf("\nROBOT's route length less than that expressed in
        chromosome. \n"); printf("\n chromDist: %f, routeDist: %f \n",
        chromDist, robot5Dis); printf("\n chromSegment: "); index = (robot5 + 1)
        % (pj->chromLen); while (index != robot1)
                {
                        printf("%d ", pj->chrom[index]);
                        printf("( %d, %d ), ", edges[pj->chrom[index]].start,
        edges[pj->chrom[index]].end); index = (index + 1) % (pj->chromLen);
    ...
    // routeDistTotal += robot1Dis + robot2Dis + robot3Dis + robot4Dis +
    // robot5Dis;	//For non min-max problem
...
void AppInit(char *appInfile, Population *p) {
  ...
  // initialize dijkstra table
  // dynamic memory allocation allows us to surpass an arbitrary memory limit
  ...
  for (int i = 0; i < NUMBER_POINTS; i++)
  {
          ...
          for (int j = 0; j < NUMBER_POINTS; j++)
          {
                  dijkstraRouteTable[i][j] = (int
  *)malloc(MAX_ROUTE_LENGTH*sizeof(int));
          }
  }*/
...
// route construct
void PhenoRoutePrint(IPTR pj, int posStart, int posEnd, FILE *fp,
                     int *currentPoint) {
...
Tuple PhenoRouteGet(IPTR pj, int posStart, int posEnd, int *currentPoint) {
  ...
  for (int i = 0; i < MAX_ROUTE_LENGTH - 1; i++) {
    if (myTuple.route[i] != -1 && myTuple.route[i + 1] != -1) {
      myDist +=
          graph[myTuple.route[i]][myTuple.route[i + 1]] *
...
void PhenoPrint(FILE *fp, IPTR pop, Population *p)  // modified
{
  ...
  // --
  if (robot2 - robot1 <= 2 || (robot2 - robot1) >= p->chromLength - 2 ||
      robot3 - robot2 <= 2 || robot3 - robot2 >= p->chromLength - 2 ||
      robot4 - robot3 <= 2 || robot4 - robot3 >= p->chromLength - 2 ||
      robot5 - robot4 <= 2 || robot5 - robot4 >= p->chromLength - 2 ||
      robot5 - robot1 >= p->chromLength - 2) {
    fprintf(fp, "Illegal Arrangement");
    return;
  }

  int currentPoint1 = getStartPoint(pop, (robot1 + 1) % pop->chromLen,
                                    (robot1 + 2) % pop->chromLen);
  // route distance from robot1+1 to robot2
  fprintf(fp, "Robot-1=>");
  PhenoRoutePrint(pop, robot1 + 1, robot2, fp, &currentPoint1);

  int currentPoint2 = getStartPoint(pop, (robot2 + 1) % pop->chromLen,
                                    (robot2 + 2) % pop->chromLen);
  // route distance from robot2+1 to robot3
  fprintf(fp, "Robot-2=>");
  PhenoRoutePrint(pop, robot2 + 1, robot3, fp, &currentPoint2);

  int currentPoint3 = getStartPoint(pop, (robot3 + 1) % pop->chromLen,
                                    (robot3 + 2) % pop->chromLen);
  // route distance from robot3+1 to robot4
  fprintf(fp, "Robot-3=>");
  PhenoRoutePrint(pop, robot3 + 1, robot4, fp, &currentPoint3);

  int currentPoint4 = getStartPoint(pop, (robot4 + 1) % pop->chromLen,
                                    (robot4 + 2) % pop->chromLen);
  // route distance from robot4+1 to robot5
  fprintf(fp, "Robot-4=>");
  PhenoRoutePrint(pop, robot4 + 1, robot5, fp, &currentPoint4);

  int currentPoint5 = getStartPoint(pop, (robot5 + 1) % pop->chromLen,
                                    (robot5 + 2) % pop->chromLen);
  // route distance from robot5 to chromLen, part 1
  fprintf(fp, "\nRobot-5=>");
  // in case robot5 is the last element in the chromosome
  if (robot5 != pop->chromLen - 1) {
    PhenoRoutePrint(pop, (robot5 + 1) % pop->chromLen, pop->chromLen, fp,
                    &currentPoint5);
  }
  // route distance from 0 to robot1, part 2
