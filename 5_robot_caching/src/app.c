...
const double DEADHEADING_RATIO = 1.0;  // Ratio of deadheading cost to service cost (can be adjusted
                                       // to reflect real speeds of robot)
...
int getStartPoint(IPTR pj, int p0, int p1);
...
int getStartPoint(IPTR pj, int p0,
	...
	int p1e = edges[pj->chrom[p1]].end;  // posibly going past the max length since robots index
	...
	} else if (p0e == p1s || p0e == p1e)  // if 0e=1s or 0e=1e
	{
		...
		if (d00 == _max || d01 == _max) {              // if 0s - 1s or 1e is longest start at
			                                           // 0s
			return p0s;
		} else if (d10 == _max || d11 == _max) {  // if 0e - 1s/1e is longest start at 0e
			return p0e;
		}
	}
...
Tuple routeDistance(IPTR pj, int posStart, int posEnd, int *currentPoint) {
	...
	for (int i = posStart + 1; i < posEnd; i++) {
		...
		currentDistance += graph[edges[pj->chrom[i]].start][edges[pj->chrom[i]].end] *
	...
	if (DIJKSTRA_INITIALIZED_FLAG == false) {
		...
		for (int r = 0; r < NUMBER_POINTS; r++) {
			for (int c = 0; c < NUMBER_POINTS; c++) {
				for (int i = 0; i < MAX_ROUTE_LENGTH; i++) { dijkstraRouteTable[r][c][i] = -1; }
...
Tuple PhenoRouteGet(IPTR pj, int posStart, int posEnd, int *currentPoint) {
	...
	for (int i = 0; i < MAX_ROUTE_LENGTH - 1; i++) {
		if (myTuple.route[i] != -1 && myTuple.route[i + 1] != -1) {
			myDist += graph[myTuple.route[i]][myTuple.route[i + 1]] *
...
void PhenoPrint(FILE *fp, IPTR pop, Population *p)  // modified
{
	...
	int currentPoint1 =
	    getStartPoint(pop, (robot1 + 1) % pop->chromLen, (robot1 + 2) % pop->chromLen);
	// route distance from robot1+1 to robot2
	fprintf(fp, "Robot-1=>");
	PhenoRoutePrint(pop, robot1 + 1, robot2, fp, &currentPoint1);

	int currentPoint2 =
	    getStartPoint(pop, (robot2 + 1) % pop->chromLen, (robot2 + 2) % pop->chromLen);
	// route distance from robot2+1 to robot3
	fprintf(fp, "Robot-2=>");
	PhenoRoutePrint(pop, robot2 + 1, robot3, fp, &currentPoint2);

	int currentPoint3 =
	    getStartPoint(pop, (robot3 + 1) % pop->chromLen, (robot3 + 2) % pop->chromLen);
	// route distance from robot3+1 to robot4
	fprintf(fp, "Robot-3=>");
	PhenoRoutePrint(pop, robot3 + 1, robot4, fp, &currentPoint3);

	int currentPoint4 =
	    getStartPoint(pop, (robot4 + 1) % pop->chromLen, (robot4 + 2) % pop->chromLen);
	// route distance from robot4+1 to robot5
	fprintf(fp, "Robot-4=>");
	PhenoRoutePrint(pop, robot4 + 1, robot5, fp, &currentPoint4);

	int currentPoint5 =
	    getStartPoint(pop, (robot5 + 1) % pop->chromLen, (robot5 + 2) % pop->chromLen);
	// route distance from robot5 to chromLen, part 1
	fprintf(fp, "\nRobot-5=>");
