...
void GetInputParameters(Population *p, Functions *f) { /* initialize Population Params */
	...
	printf(" Enter kind of crossover (0(npoint),1(PMX),2(OX), 3(Greedy),..)-> "); /* Assume binary
	                                                                                 Xover*/
	...
	if (p->xType == 0) printf(" Xover: Simple N point where n is %d \n", p->nXPoints);
	...
	if (tmpint == 1) printf(" GAFlavor: CHC with lambda %d \n", p->lambda);
	skipline(inpfl);
	...
	if (p->injectFraction <= 0.0) { printf(" I am NOT going to use the case-base\n "); }
...
void Initialize(int argc, char *argv[], Population *p, Functions *f) {
	...
	if (p->iFile != NULL) { GetInputParameters(p, f); }
	...
	p->nCases        = 0;
	p->nCurrentCases = 0;
	if (p->injectFraction > 0.0) { p->nCases = FindNCases(p->nCFile); }
	p->bigMaxi = -1;
...
void Usage() {
	fprintf(stderr, "ga -i infile -o ofile -a application file -s seed -v verbose -M\n");
}
...
void InitGA(Population *p, Functions *f) {
	...
	if (p->injectFraction > 0.0) {
		InitLoadCases(p->caseFileName, p->oldpop, p->generation, p->injectFraction, p);
